...
def login(session: Session,
    ...
    def get_login_page():
        return session.get(
            url=AUTHSERVER_URL, #换成这个网址
            params=None if service is None else {"service": service},
            allow_redirects=False,
            timeout=timeout)
    login_page = get_login_page() #获得登录页面
    ...
    if login_page.status_code == 302:
        ...
        raise UnknownAuthserverException() #抛出登录或服务过程中未知错误
    try:
    ...
    def after_captcha(captcha_str: Optional[str]):
        ...
        login_resp = session.post(
            url=AUTHSERVER_URL, data=formdata, allow_redirects=False) #登录请求,提交信息
        
        #重定向到服务页面
        def redirect_to_service():
            # print(requests.utils.dict_from_cookiejar(session.cookies))
            session.get(url=login_resp.headers['Location'], allow_redirects=False)
            return session
        
        if login_resp.status_code != 302: #为302表示重定向
...
keep_longer= False
kick_others = False
se=login(session=session,username=username,password=password,
      service=None,timeout=10,force_relogin=False,captcha_callback=None,keep_longer=False,kick_others = False)
# access_sso_service(se,service)
res=access_service(se,service)
print(res.text)
